...
export const PostCard = ({ post, onPostUpdated, onPostDeleted }) => {
  const { user } = useAuth();
  const initialLikes = Array.isArray(post.likes) ? post.likes : [];
  const [isLiked, setIsLiked] = useState(initialLikes.includes(user?.id || ''));
  ...
  const handleLike = async () => {
    try {
      if (isLiked) {
        await postAPI.unlike(post._id, user?.id);
        setLikesCount((prev) => prev - 1);
      } else {
        await postAPI.like(post._id, user?.id);
        setLikesCount((prev) => prev + 1);
      }
      setIsLiked(!isLiked);
    } catch (error) {
        ...
        <div className="post-author">
          ...
          <div className="author-info">
            ...
            <span className="author-title">
              {post.author?.designation || post.author?.userType || 'User'}
