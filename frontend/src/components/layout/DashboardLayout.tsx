...
interface NavItem { path: string; label: string; icon: React.ReactNode; badge?: string | number; badgeType?: 'default' | 'new'; }
interface SidebarStat { label: string; value: string; color: 'default' | 'blue' | 'orange'; }
...
export const DashboardLayout: React.FC = () => {
  ...
  const isHomeFeedPage = location.pathname === '/home';
  const canPost = user?.userType === 'faculty' || user?.userType === 'admin';

  const handleLogout = () => { logout(); navigate('/login'); };
  const handlePostCreated = () => setShowCreatePost(false);
  ...
  useEffect(() => {
    ...
    const loadHeaderAvatar = async () => {
      if (!user?.id) { setHeaderAvatarUrl(null); return; }
      try {
        if (user.userType === 'student') {
          const profile = await studentAPI.getProfile(user.email || user.id);
          setHeaderAvatarUrl(resolveImageUrl(profile.profile_image || null));
          return;
        }
        if (user.userType === 'faculty') {
          const profile = await facultyAPI.getProfile();
          setHeaderAvatarUrl(resolveImageUrl((profile as any)?.profilepic || (profile as any)?.profile_image || null));
          return;
        }
        setHeaderAvatarUrl(null);
      } catch { setHeaderAvatarUrl(null); }
    };
    loadHeaderAvatar();
  ...
  useEffect(() => {
    const loadSidebarStats = async () => {
      if (!user?.id) { setSidebarStats([]); return; }
      try {
        if (user.userType === 'faculty' || user.userType === 'admin') {
          ...
          setSidebarStats([
            ...
            { label: 'Pending Reviews', value: String(Math.max(totalStudents - verifiedProfiles, 0)), color: 'orange' },
          ]);
          return;
        }
        const profile = await studentAPI.getProfile(user.email || user.id);
        const skillsVerified = Array.isArray(profile?.skills) ? profile.skills.filter((skill) => Boolean(skill?.verified)).length : 0;
        setSidebarStats([{ label: 'Skills Verified', value: String(skillsVerified), color: 'default' }]);
      } catch {
        setSidebarStats(user.userType === 'faculty' || user.userType === 'admin'
          ? [{ label: 'Active Students', value: '0', color: 'default' }, { label: 'Verified Profiles', value: '0', color: 'blue' }, { label: 'Pending Reviews', value: '0', color: 'orange' }]
  ...
  const getNavItems = (): NavItem[] => {
    ...
    if (userType === 'admin') return [
      { path: '/home', label: 'Home Feed', icon: <Home size={20} /> }, { path: '/chat', label: 'Messages', icon: <MessageSquare size={20} /> },
      { path: '/admin/students', label: 'Manage Students', icon: <Users size={20} /> }, { path: '/admin/faculty', label: 'Manage Faculty', icon: <GraduationCap size={20} /> },
      { path: '/admin/analytics', label: 'Analytics', icon: <TrendingUp size={20} /> }, { path: '/admin/settings', label: 'Settings', icon: <Settings size={20} /> },
    ];
    if (userType === 'faculty') {
      const facultyItems: NavItem[] = [
        { path: '/home', label: 'Home Feed', icon: <Home size={20} /> }, { path: '/chat', label: 'Messages', icon: <MessageSquare size={20} /> },
        { path: '/faculty/search', label: 'Smart Search', icon: <Search size={20} /> }, { path: '/faculty/verification', label: 'Facial Recognition', icon: <Camera size={20} />, badge: 'New', badgeType: 'new' },
        { path: '/faculty/recommendations', label: 'Recommendations', icon: <Star size={20} />, badge: 5 }, { path: '/faculty/analytics', label: 'Analytics', icon: <TrendingUp size={20} /> },
      ];
      if (hasAdminManagementAccess) facultyItems.push(
        { path: '/admin/students', label: 'Manage Students', icon: <Users size={20} /> }, { path: '/admin/faculty', label: 'Manage Faculty', icon: <GraduationCap size={20} /> },
        { path: '/admin/analytics', label: 'Admin Analytics', icon: <TrendingUp size={20} /> }, { path: '/admin/department/audit-logs', label: 'Department Logs', icon: <ClipboardList size={20} /> },
  ...
  const getUserDisplayName = () => user?.name?.trim() || `${user?.firstname || ''} ${user?.lastName || ''}`.trim() || (user?.userType === 'faculty' ? 'Faculty' : 'User');
  const getUserInitials = () => { const name = getUserDisplayName(); const parts = name.split(' '); return parts.length >= 2 ? `${parts[0][0]}${parts[parts.length - 1][0]}`.toUpperCase() : name.substring(0, 2).toUpperCase(); };
