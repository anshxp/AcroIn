...
export const DashboardLayout: React.FC = () => {
  ...
  const getUserDisplayName = () => {
    ...
    if (user?.firstname || user?.lastName) return `${user?.firstname || ''} ${user?.lastName || ''}`.trim() || 'User';
    ...
    <div className="dashboard">
      <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
        <div className="sidebar-header">
          <NavLink to="/" className="sidebar-logo"><div className="sidebar-logo-icon">AI</div><span className="sidebar-logo-text">Acro-In</span></NavLink>
          <button className="sidebar-toggle" onClick={() => setCollapsed(!collapsed)}><ChevronLeft size={16} style={{ transform: collapsed ? 'rotate(180deg)' : 'none' }} /></button>
        </div>
        <nav className="sidebar-nav"><div className="nav-section">
          {navItems.map((item) => (
            <NavLink key={item.path} to={item.path} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
              {item.icon}<span className="nav-item-text">{item.label}</span>
              {item.badge && <span className={`nav-badge ${item.badgeType === 'new' ? 'new' : ''}`}>{item.badge}</span>}
            </NavLink>
          ))}
        </div></nav>
        <div className="sidebar-footer"><div className="sidebar-stats">
          {sidebarStats.map((stat, index) => <div key={index} className="sidebar-stat"><span className="sidebar-stat-label">{stat.label}</span><span className={`sidebar-stat-value ${stat.color}`}>{stat.value}</span></div>)}
        </div></div>
      </aside>
      ...
      <div className="main-content">
        <header className="top-header">
          <form className="search-bar" onSubmit={handleSearchSubmit}>
            <Search size={20} />
            <input type="text" placeholder={user?.userType === 'student' ? 'Search...' : 'Search students, skills, projects...'} value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} />
          </form>
          <div className="header-actions">
            <button className="header-icon-btn" onClick={() => navigate('/notifications')} type="button" title="Notifications"><Bell size={20} />{unreadCount > 0 && <span className="notification-badge">{unreadCount > 99 ? '99+' : unreadCount}</span>}</button>
            <div className="user-profile" onClick={() => navigate(user?.userType === 'faculty' ? '/faculty/profile' : '/student/profile')}>
              <div className="user-avatar">{headerAvatarUrl ? <img src={headerAvatarUrl} alt="User profile" className="user-avatar-image" /> : getUserInitials()}</div>
              <div className="user-info"><span className="user-name">{getUserDisplayName()}</span><span className="user-role">{user?.userType === 'faculty' ? 'Faculty' : user?.userType === 'admin' ? 'Admin' : 'Student'}</span></div>
            </div>
            <button className="header-icon-btn logout-btn" onClick={handleLogout} title="Logout" style={{ marginLeft: '8px', color: '#ef4444' }}><LogOut size={20} /></button>
          </div>
        </header>
