...
export const StudentCompetitions: React.FC = () => {
  ...
  const { data, loading, error } = useQuery<{ competitions: Competition[] }>(GET_COMPETITIONS);

  if (loading) return <div>Loading...</div>;
  if (error) return <div>Error: {error.message}</div>;

  const dbCompetitions = data?.competitions || [];
  const allCompetitions = [...competitions, ...dbCompetitions];
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this competition?')) {
      setCompetitions(competitions.filter((c: Competition) => c._id !== id));
    }
  };
