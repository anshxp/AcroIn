  ...
  return <div>
    ...
    <div className="student-list">
      {loading ? <p>Loading students...</p> : filteredStudents.map(student=><div key={student.id} className="student-card">
        ...
        <div className="student-actions"><div className="student-rating"><Star size={16} fill="#fbbf24"/>{student.rating}</div><button className="view-profile-btn" type="button" onClick={()=>navigate(isStudentUser?`/student/profile/${student.id}`:`/faculty/student/${student.id}`)}>{isStudentUser?'View Profile':'View Profile'}</button></div>
      </div>)}
