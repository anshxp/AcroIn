...
const toTeammate = (student: RecommendationItem, index: number): SuggestedTeammate => ({
  ...
  skills: student.matched_skills?.length
    ? student.matched_skills.slice(0, 4)
    : [],
  ...
  useEffect(() => {
    ...
    const loadRecommendations = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setSuggestedTeammates(response.data.map(toTeammate));
        }
      } catch {
        if (!cancelled) {
      ...
      <div className="dashboard-grid full">
        <div className="card">
          ...
          <div className="card-body">
            ...
            {!loading && !error && !suggestedTeammates.length && (
              ...
              >
                {suggestedTeammates.map((teammate, index) => (
                  <div
                    ...
                    style={{ padding: '20px', background: '#f8fafc', borderRadius: '12px' }}
                  >
                    ...
                    <div className="teammate-info">
                      ...
                      {teammate.missingSkills.length > 0 && (
                        <small style={{ color: '#64748b' }}>
                          Missing: {teammate.missingSkills.slice(0, 3).join(', ')}
                        </small>
                      )}
                    </div>
                    <div className="teammate-actions">
                      ...
                      <button
                        ...
                        disabled={teammate.id.startsWith('recommendation-')}
                      >
                        View Profile
                      </button>
                    </div>
