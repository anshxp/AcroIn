    ...
    <div className="page-container">
      ...
      <div className="card"><div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: '18px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'grid', gap: '7px' }}>
          ...
          <span>{faculty.designation || 'Faculty'}{faculty.department ? ` • ${faculty.department}` : ''}</span>
          {faculty.location && <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><MapPin size={14} />{faculty.location}</span>}
          {faculty.email && <a href={`mailto:${faculty.email}`} className="btn-secondary" style={{ width: 'fit-content' }}><Mail size={16} /> Contact</a>}
        </div>
      </div></div>
