...
const ALLOWED_POST_CREATORS = new Set(['student', 'faculty', 'admin']);
...
const getAuthorFromUser = async (userId) => {
  ...
  return { _id: user._id, name: user.name, designation: user.designation, department, profileImage, userType: user.userType };
};
...
const getFacultyAdminContext = async (req) => {
  ...
  return { user, faculty, isDepartmentAdmin: isFacultyDepartmentAdmin(faculty) };
};

const canFacultyModerateStudentAuthor = (facultyContext, author) => {
  if (!facultyContext?.isDepartmentAdmin || !author || author.userType !== 'student') return false;
...
const resolveAuthorDepartment = async (author) => {
  ...
  if (author.userType === 'student') return (await Student.findOne({ email: user.email }).select('department'))?.department || '';
  if (author.userType === 'faculty') return (await Faculty.findOne({ email: user.email }).select('department'))?.department || '';
  return '';
};
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (!isOwner && !isAdminUser && !canModerate) return res.status(403).json({ message: 'Not authorized to update this post' });
...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    if (user?.userType === 'student' && user.email) userDepartment = (await Student.findOne({ email: user.email }).select('department'))?.department;
    else if (user?.userType === 'faculty' && user.email) userDepartment = (await Faculty.findOne({ email: user.email }).select('department'))?.department;

    const filters = [{ scope: 'campus' }];
    if (userDepartment) filters.push({ scope: 'department', visibleToDepartments: userDepartment });
...
router.post('/', verifyToken, postUpload.array('files', 4), async (req, res) => {
  try {
    if (!ALLOWED_POST_CREATORS.has(req.user?.userType)) return res.status(403).json({ message: 'You are not authorized to create posts' });

    const { content, images, linkedOpportunity, scope } = req.body || {};
    const uploadedMedia = Array.isArray(req.files)
      ? req.files.map((file) => toAbsoluteUploadUrl(req, file.path || `/uploads/${file.filename}`)).filter(Boolean)
      : [];
    const bodyImages = Array.isArray(images) ? images.filter((image) => typeof image === 'string' && image.trim()) : [];
    const normalizedContent = typeof content === 'string' ? content.trim() : '';
    if (!normalizedContent && uploadedMedia.length === 0 && bodyImages.length === 0) return res.status(400).json({ message: 'Post content or at least one file is required' });
    ...
    if (req.user?.userType === 'faculty' && scope) {
      ...
      if (['campus', 'department'].includes(normalizedScope)) {
        ...
        if (normalizedScope === 'department' && author.department) visibleToDepartments = [author.department];
      }
    }
...
const deleteCommentHandler = async (req, res) => {
  try {
    ...
    const canModerate = facultyContext?.isDepartmentAdmin && normalizeText(commentDepartment) && normalizeText(commentDepartment) === normalizeText(facultyContext.faculty.department);
    if (!isCommentOwner && !isPostOwner && !isAdminUser && !canModerate) return res.status(403).json({ message: 'Not authorized to delete this comment' });
...
router.delete('/:id', verifyToken, async (req, res) => {
  try {
    ...
    const facultyContext = await getFacultyAdminContext(req);
    const canModerate = canFacultyModerateStudentAuthor(facultyContext, post.author);
    if (!isOwner && !isAdminUser && !canModerate) return res.status(403).json({ message: 'Not authorized to delete this post' });
